...
#tail -n 100 ~/Scripts/logs/log.rclone | grep Transferred | tail -n 2 | head -n 1
...
echo "::Data::  Restore:Drobo" > $message
echo $msg1 >> $message
#echo $msg2 >> $message
echo $msg3 >> $message
echo $msg4 >> $message
...
tail -n 100 ~/Scripts/logs/log.rclone | grep -A 1 Transferring | tail -n 2 >> $message
cat $message
curl -s \
